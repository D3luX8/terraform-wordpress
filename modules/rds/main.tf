...
########## DB-Instance ###########
resource "aws_db_instance" "db_wp-tf" {
  ...
  db_name           = var.ssm_values.db_name
  username          = var.ssm_values.db_username
  password          = var.ssm_values.db_password
  # parameter_group_name = "db_wp-tf.mysql8.0"
...
resource "aws_ssm_parameter" "endpoint" {
  ...
  type        = "SecureString"
  value       = aws_db_instance.db_wp-tf.endpoint
